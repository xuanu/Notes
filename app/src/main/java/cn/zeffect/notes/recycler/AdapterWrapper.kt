...
class AdapterWrapper<T : RecyclerView.ViewHolder>(adapter: RecyclerView.Adapter<T>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    fun isHeader(position: Int): Boolean = position < mHeaderViews.size()
    fun isFooter(position: Int): Boolean = position >= mHeaderViews.size() + mAdapter.itemCount

    fun isEmpty(): Boolean = mAdapter.itemCount == 0
    ...
    private fun generateEmpty(context: Context) = LinearLayout(context).apply {
        ...
        addView(TextView(context).apply { text = "无数据";textColor = Color.parseColor("#aaaaaa") })
    }
}